...
export default function Navbar({ isMobile = false }: { isMobile?: boolean }) {
  ...
  // Hide the navbar on all blog pages (e.g., /blog and /blog/anything)
  ...
  return (
    <nav
      className={`${isMobile ? "fixed" : "sticky"} top-0 left-0 right-0 z-50 w-full border-b border-white/10 bg-black/90 backdrop-blur-md supports-[backdrop-filter]:bg-black/70 ${isMobile ? "pt-safe-top" : ""}`}
    >
              ...
              <div className="grid gap-2 text-sm">
                ...
                {session?.user && (
                  <button
                    ...
                    className="rounded-xl border border-white/10 px-3 py-2 text-left text-white/90 hover:bg-white/10 mobile-touch-target"
                  >
                    🚪 Logout
                  </button>
                )}
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[70] p-4">
            <div className="bg-slate-900 border border-white/20 rounded-xl p-6 max-w-md w-full">
              ...
              <div className="flex gap-3">
                <Button onClick={handleCancelSignOut} variant="secondary" className="flex-1">
                  Cancel
                </Button>
                <Button
...
function LinkItem({ href, label, onClick }: { href: string; label: string; onClick?: () => void }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="rounded-xl border border-white/10 px-3 py-2 text-white/90 hover:bg-white/10 transition-colors mobile-touch-target"
    >
      {label}
    </Link>
  );
