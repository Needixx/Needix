...
export default function SettingsClient({ user }: { user: User }) {
  ...
  const deleteAccount = async () => {
    ...
    alert('Account deletion is not yet implemented. Please contact support at needix2025@gmail.com to delete your account.');
  };
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6">
        ...
        <div className="mt-4 flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${isPro ? 'bg-cyan-400' : 'bg-green-500'}`} />
          <span className="text-sm text-white/70">
            {isPro ? 'Needix Pro' : 'Free Plan'} • {subsData.items.length} subscription{subsData.items.length !== 1 ? 's' : ''} • {ordersData.items.length} order{ordersData.items.length !== 1 ? 's' : ''}
          </span>
        </div>
      </section>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="grid gap-2">
            ...
            <select
              ...
              onChange={(e) => updateAppSettings({ theme: e.target.value as "dark" | "light" | "system" })}
            >
          ...
          <label className="grid gap-2">
            ...
            <select
              ...
              onChange={(e) => updateAppSettings({ currency: e.target.value as "USD" | "EUR" | "GBP" })}
            >
          ...
          <label className="grid gap-2">
            ...
            <select
              ...
              onChange={(e) => updateAppSettings({ defaultView: e.target.value as "grid" | "list" })}
            >
