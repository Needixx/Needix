...
const [items, setItems] = useState<SubscriptionItem[]>(demoSubscriptions);
const total = useMemo(() => items.reduce((acc, i) => acc + i.cost, 0), [items]);
...
<p className="text-sm text-white/60">Renews {i.nextPayment}</p>
</div>
<div className="text-right">
<p className="font-semibold">${i.cost.toFixed(2)}</p>
</div>
</div>
</div>
))}
