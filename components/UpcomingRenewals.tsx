...
export default function UpcomingRenewals() {
  ...
  const [sortKey, setSortKey] = useState<"date_asc" | "date_desc" | "name_asc" | "price_desc">("date_asc");
  ...
  function snooze(sub: Subscription, days: number) {
    const base = sub.nextBillingDate ? new Date(`${sub.nextBillingDate}T00:00:00`) : new Date();
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="flex items-center justify-between rounded-xl border border-white/10 bg-neutral-800 px-3 py-2">
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={hideNoDate} onChange={(e) => setHideNoDate(e.target.checked)} />
            Hide without date
          </label>
          <select
            className="bg-transparent outline-none text-sm"
            value={sortKey}
            onChange={(e) => setSortKey(e.target.value as "date_asc" | "date_desc" | "name_asc" | "price_desc")}
          >
      ...
      <div className="overflow-x-auto rounded-2xl border border-white/10">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {sorted.map((s) => {
              const date = s.nextBillingDate ? new Date(`${s.nextBillingDate}T00:00:00`) : null;
              const days = date ? Math.ceil((date.getTime() - nowRef.getTime()) / 86400000) : null;
              return (
                <tr key={s.id} className="border-t border-white/10">
                  <td className="px-3 py-2">
                    <div className="font-medium">{s.name}</div>
                    <div className="text-xs text-white/50">{s.category || 'Uncategorized'}</div>
                  </td>
                  <td className="px-3 py-2">
                    {date ? (
                      <div>
                        <div>{date.toLocaleDateString()}</div>
                        <div className="text-xs text-white/50">{days! >= 0 ? `in ${days} day${days === 1 ? '' : 's'}` : `${Math.abs(days!)} days ago`}</div>
                      </div>
                    ) : (
                      <span className="text-white/50">—</span>
                    )}
                  </td>
                  <td className="px-3 py-2">{fmtCurrency(s.price, s.currency)}</td>
                  <td className="px-3 py-2 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <Button variant="secondary" onClick={() => snooze(s, 1)}>Snooze 1d</Button>
                      <Button variant="secondary" onClick={() => snooze(s, 3)}>3d</Button>
                      <Button variant="secondary" onClick={() => snooze(s, 7)}>7d</Button>
                      <Button variant="ghost" onClick={() => setEditing(s)}>Edit</Button>
                      <Button variant="ghost" onClick={() => { if (confirm(`Delete ${s.name}?`)) { remove(s.id); toast(`Deleted ${s.name}`, 'success'); } }}>Delete</Button>
                    </div>
                  </td>
                </tr>
              );
            ...
            {sorted.length === 0 && (
              <tr>
                <td colSpan={4} className="px-3 py-8 text-center text-white/60">No renewals match your filters.</td>
              </tr>
            )}
          </tbody>
        </table>
